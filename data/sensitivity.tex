...
其中$E_\nu$为中微子能量，$T$为核反冲能，$M$为原子核质量，$G_F$为费米常数，其他相关参数的定义见式\ref{eq:cevns_theta}。
...
某一次测量的参数置信区间如图\ref{fig:nsi_sensitivity_s2only}，其中``RELICS''为预期的LXeTPC实验的名字，
...
\begin{figure}
  \centering
  \includesvg[width=0.7\linewidth]{figures/exp_s1s2_t1t2.svg}
...
某一次测量的参数置信区间如图\ref{fig:nsi_sensitivity_s1s2}，
同时列出了COHERENT的首次结果作为对比\cite{akimov_observation_2017}。
...
使用$5\si{kg\cdot y}$\ HPGe的测量结果，置信区间非常窄，因为$\mathrm{Ge}$原子质量较低，对于相同能量的入射中微子，
核反冲平均能量比使用$\mathrm{Xe}$进行的测量高，如图\ref{fig:cevns_event_rate}，对测量非常有利。
...
建立$\chi^2$，如式\ref{eq:thets_s2_ll}。
...
对比红色实线和蓝色虚线，淬灭因子系统误差对灵敏度的影响也很大。使用$5\si{kg\cdot y}$\ HPGe的测量结果优于液氙的测量结果，因为探测器阈值附近的信噪比更高，
如图\ref{fig:event_smear_Ge_rate_prediction}，且HPGe探测器的能量分辨率更高。

\section{低能核反冲的液氙光电产额测量}

在标准模型框架内，利用反应堆中微子的CE$\nu$NS较高统计量，可以对LXeTPC的光电产额进行标定。
...
图\ref{fig:qy_sensitivity_s2only}红线所示精度的测量结果将对暗物质实验和基于液氙的太阳中微子实验有提升作用。

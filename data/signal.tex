...
HPGe探测器对核反冲有一定的猝灭效应，猝灭因子$Q_F$与可视能量的关系为式\ref{eq:vis_q}。
...
其中$T_\mathrm{vis}$为可视能量，单位一般写为$\si{keV_{ee}}$。猝灭因子可以从理论计算或实际测量得到\cite{zhao_search_2016}，
本文中选用的猝灭因子及其不确定度如图\ref{fig:ge_quenching_factor}。

\begin{figure}
  \begin{subfigure}{.5\textwidth}
    \centering
    \includesvg[width=1.0\linewidth]{figures/ge_quenching_factor.svg}
    \caption{\label{fig:ge_quenching_factor} HPGe探测器的猝灭因子，及不确定度\cite{zhao_search_2016}}
  \end{subfigure}
  \begin{subfigure}{.5\textwidth}
    \centering
  ...
  \end{subfigure}
  \caption{\label{fig:ge_qf_reso} $\si{kg}$量级的pPCGe探测器猝灭因子和能量分辨率}
\end{figure}
